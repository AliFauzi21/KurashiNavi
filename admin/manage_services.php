...
if($_SERVER['REQUEST_METHOD'] == 'POST') {
    if(isset($_POST['action'])) {
        try {
            switch($_POST['action']) {
                case 'add':
                    $stmt = $conn->prepare("INSERT INTO services (title, description, status) VALUES (?, ?, ?)");
                    $stmt->execute([$_POST['title'], $_POST['description'], $_POST['status']]);
                    break;
                case 'edit':
                    $stmt = $conn->prepare("UPDATE services SET title = ?, description = ?, status = ? WHERE id = ?");
                    $stmt->execute([$_POST['title'], $_POST['description'], $_POST['status'], $_POST['id']]);
                    break;
                case 'delete':
...
<body>
    <div class="admin-container">
        <div class="sidebar">
            ...
            <ul class="sidebar-menu">
                ...
                <li><a href="manage_services.php" class="active">サービス管理</a></li>
        ...
        <div class="main-content">
            <div class="content-header">
                <h1>サービス管理</h1>
                <button class="add-button" onclick="showAddModal()">新規サービス追加</button>
            </div>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="action-buttons">
                            <button class="edit-button" onclick="showEditModal(<?php echo htmlspecialchars(json_encode($service)); ?>)">編集</button>
                            <button class="delete-button" onclick="deleteService(<?php echo $service['id']; ?>)">削除</button>
                        </td>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
    ...
    <script>
        function showAddModal() {
            document.getElementById('modalTitle').textContent = '新規サービス追加';
