...
if($_SERVER['REQUEST_METHOD'] == 'POST') {
    if(isset($_POST['action'])) {
        try {
            switch($_POST['action']) {
                case 'add':
                    $stmt = $conn->prepare("INSERT INTO community_posts (title, content, category, status) VALUES (?, ?, ?, ?)");
                    $stmt->execute([$_POST['title'], $_POST['content'], $_POST['category'], $_POST['status']]);
                    break;
                case 'edit':
                    $stmt = $conn->prepare("UPDATE community_posts SET title = ?, content = ?, category = ?, status = ? WHERE id = ?");
...
try {
    $stmt = $conn->query("SELECT * FROM community_posts ORDER BY created_at DESC");
    $posts = $stmt->fetchAll();
} catch(PDOException $e) {
...
<body>
    <div class="admin-container">
        <div class="sidebar">
            ...
            <ul class="sidebar-menu">
                ...
                <li><a href="manage_community.php" class="active">コミュニティ管理</a></li>
        ...
        <div class="main-content">
            <div class="content-header">
                <h1>コミュニティ管理</h1>
                <button class="add-button" onclick="showAddModal()">新規投稿追加</button>
            </div>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="action-buttons">
                            <button class="edit-button" onclick="showEditModal(<?php echo htmlspecialchars(json_encode($post)); ?>)">編集</button>
                            <button class="delete-button" onclick="deletePost(<?php echo $post['id']; ?>)">削除</button>
                        </td>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
    ...
    <!-- Modal untuk tambah/edit posting -->
    <div id="postModal" class="modal">
        <div class="modal-content">
            <div class="modal-header">
                <h2 id="modalTitle">新規投稿追加</h2>
            </div>
    ...
    <script>
        function showAddModal() {
            document.getElementById('modalTitle').textContent = '新規投稿追加';
        ...
        function showEditModal(post) {
            document.getElementById('modalTitle').textContent = '投稿編集';
